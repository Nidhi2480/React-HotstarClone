        ...
        <ul>
          ...
          </li>
          {
            NavData.map((nav, index) => (
              <li key={index}>
                {nav.name === 'Favourites' ? (
                  <span className={`nav-link ${activeNav === nav.name ? 'active' : ''}`} onClick={handleClickFav}>
                    <span className="numbers">{FavCount}</span>
                    <img 
                      src={nav.icon} 
                      alt="profile" 
                      onMouseEnter={() => handleHover(nav.name)} 
                      onMouseLeave={handleLeave}
                    />
                    {isHovered && (
                      <span className={`nav-text ${activeNav === nav.name ? 'active' : ''}`}>
                        {nav.name}
                      </span>
                    )}
                  </span>
